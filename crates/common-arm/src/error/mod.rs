...
//! structs and macros to simplify and standardize error management in Rocket.
//!
//! ## RocketError
//! [`RocketError`] is the standard error type to be used throughout the Rocket codebase. It is an enum
//! that wraps all possible error types that can be encountered. This allows functions to have a
//! standard `Result<_, RocketError` that can be returned. The `From` trait is also automatically
//! implemented for all error variants, allowing the use of the `?` operator. For example:
//! ```ignore
//! fn send_message() -> Result<(), RocketError> {
...
//!
//! ## ErrorManager
//! While [`RocketError`] standardizes the error type, something must be done when an error happens.
...
//! ```
//! # use common_arm::{ErrorManager, RocketError};
//! fn foo() -> Result<(), RocketError> {
//!     // Do something that can throw an error
//!     # Ok(())
//! }
//!
...
//! [`RocketError`] while preserving useful info like the task name that failed to spawn.
//!
...
//! converting the returned `Result` into a `Result<(), RocketError>`, adding in the process some
...
pub mod rocket_error;

/// Calls `spawn` on a RTIC's task. Transforms the returned `Result` into a `Result<_, RocketError>`.
#[macro_export]
macro_rules! spawn {
    ($task:ident$(,)? $($arg:expr),*) => {
...
/// Calls `spawn_after` on a RTIC's task. Transforms the returned `Result` into a `Result<_, RocketError>`.
#[macro_export]
